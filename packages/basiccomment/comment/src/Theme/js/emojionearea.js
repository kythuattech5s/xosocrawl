...
(function (factory, global) {
    ...
    function unicodeTo(str, template) {
        return str.replace(uniRegexp, function (unicodeChar) {
            var map =
                emojione[
                    emojioneSupportMode === 0 ? "jsecapeMap" : "jsEscapeMap"
                ];
    ...
    function textFromHtml(str, self) {
        str = str
            ...
            .replace(/<br[^>]*>/gi, "\n")
            .replace(
                /(?:<(?:div|p|ol|ul|li|pre|code|object)[^>]*>)+/gi,
                "<div>"
            )
    ...
    function calcButtonPosition() {
        ...
        if (current !== offset) {
            ...
            if (self.floatingPicker) {
                self.picker.css({
                    right:
                        parseInt(self.picker.css("right")) - current + offset,
                });
            }
        }
    }
    ...
    function init(self, source, options) {
        ...
        self.inline =
            options.inline === null ? source.is("INPUT") : options.inline;
        ...
        var sourceValFunc =
            ...
            tones = div(
                ...
                options.tones
                    ? function () {
                          this.addClass(
                              selector("tones-" + options.tonesStyle, true)
                          );
                          for (var i = 0; i <= 5; i++) {
                              this.append(
                                  $("<i/>", {
            ...
            app = div(
                ...
                (picker = self.picker =
                    div(
                        ...
                        .addClass(
                            selector(
                                "picker-position-" + options.pickerPosition,
                                true
                            )
                        )
                        .addClass(
                            selector(
                                "filters-position-" + options.filtersPosition,
                                true
                            )
                        )
                        .addClass(
                            selector(
                                "search-position-" + options.searchPosition,
                                true
                            )
                        )
                        .addClass("hidden"))
        ...
        self.on("@filter.click", function (filter) {
            ...
            .on("@!paste", function (editor, event) {
                var pasteText = function (text) {
                    ...
                    if (
                        editorScrollTop + top >= height ||
                        editorScrollTop > top
                    ) {
                        editor.scrollTop(
                            editorScrollTop + top - (2 * height) / 3
                        );
                    }
                    caret.remove();
        ...
        if (!cdn.isLoading) {
            if (!emojione || getSupportMode(detectVersion(emojione)) < 2) {
                ...
                if (getSupportMode(emojioneVersion) > 5) {
                    ...
                    emojioneJsCdnUrlBase =
                        cdn.defaultBase + "/" + emojioneVersion;
                }
                ...
                $.ajax({
                    ...
                    success: function () {
                        ...
                        if (emojioneSupportMode > 4) {
                            ...
                            cdn.base =
                                cdn.defaultBase + emojioneVersion + "/assets";
